...
一些字段还是沿用[默认主题](https://vuepress.vuejs.org/zh/theme/default-theme-config.html#%E9%A6%96%E9%A1%B5)的，这里只做对修改的地方做一个补充。
...
### bannerBg
  * 类型：`string`
  * 可选参数：
    * `auto` 自动背景，一般会显示网格纹背景，如果有设置`bodyBgImg`时则无背景
    * `none` 无背景
    * `<大图地址>`，如`/img/bg.jpeg`
...
*  **features[index].link** 当前feature跳转的链接
*  **features[index].imgUrl** 当前feature的图片地址
...
### postList
  * 类型：`string`
  * 可选参数：
    * `detailed` 显示详细版文章列表（包括作者、分类、标签、摘要、分页等）
